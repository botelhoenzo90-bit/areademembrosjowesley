...
import { ArrowLeft, Play, Clock, Sparkles, Flame, ChevronRight, Activity, Target, Shield, Compass } from "lucide-react";
...
function Page() {
  ...
  const { data: userArchetypes = [] } = useSuspenseQuery({
    queryKey: ['hero_journey_archetypes'],
    queryFn: () => getArchetypes(),
  });
  ...
  useEffect(() => {
    ...
    // Show splash if it's the first time
  ...
  const getStatus = (id: string) => {
    ...
    if (arch) return arch.status as any;
    
    // Logic for sequence unlocking
    ...
    <main className="relative min-h-screen pb-24 bg-background">
      ...
      <section className="mx-4 mt-12 lg:mx-auto lg:max-w-6xl">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {/* Mock cards for locked ones not in content yet */}
      ...
      {/* Original Lesson Section (Preserved) */}
